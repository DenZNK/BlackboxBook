...
На уровне 2026 года архитектура Generator + Verifier стала стандартом в продакшн-системах. Anthropic встроили эту идею в **Constitutional AI**: модель сначала генерирует, потом сама оценивает ответ через набор принципов (constitution), и переписывает его. **RLAIF** (Reinforcement Learning from AI Feedback) обучает модель на таких самопроверках ещё на этапе тренировки — до деплоя.
...
| Метод | Дополнительная задержка | Дополнительная стоимость | Снижение галлюцинаций |
|-------|------------------------|-------------------------|----------------------|
| Второй LLM-вызов (Verifier) | +1–3 сек | ×2 стоимость | 30–50% |
| Полный CoVe (4 этапа) | +5–15 сек | ×3–5 стоимость | 40–70% |
...
|--------|---------------|----------|
| **Hallucination rate** (скользящее окно 24ч) | % ответов с подтверждёнными ошибками | >5% |
