# ГЛАВА 15. КАК НАЧАТЬ: ОТ ПЕРВОГО ПРОМПТА ДО РАБОЧЕГО АГЕНТНОГО КОНТУРА
...
## 15.1. Где агентный подход уже выгоден
...
## 15.2. Выбор модели: дерево решений
...
| Production: код, рассуждения | Claude Sonnet/Opus 4.6, GPT-5.4 (reasoning.effort=high) | Тесты, long-context, tool reliability |
...
## 15.3. Инфраструктура: API, self-hosted или гибрид
...
- **Модели**: Llama 4 Scout, Llama 4 Maverick, Qwen3, DeepSeek-V3, GLM-5.1, Gemma 4. Конкретный выбор определяется не пресс-релизом, а вашим latency/cost/SLA-профилем.
...
## 15.4. Минимальный агентный контур
...
## 15.5. Как вводить ИИ постепенно
...
## 15.6. Метрики, которые нужно измерять
...
## 15.7. Типичные ошибки при внедрении
...
**Проблема**: агентный контур с 3 итерациями по 4 вызова LLM × 3000 токенов = 36K токенов на запрос. При 10K запросов/день = 360M токенов/день = **$1800/день** (frontier-модель со средней ценой ~$5/MTok).
...
## 15.8. Минимальный запуск: чек-лист первой недели
...
## 15.10. Архитектура развёртывания
...
**Навигация:**
- Назад: [Глава 14. Архитектура кода, дружественная ИИ](14_архитектура_кода.md)
- Далее: [Глава 16. Ландшафт 2026](16_ландшафт_2026.md)
