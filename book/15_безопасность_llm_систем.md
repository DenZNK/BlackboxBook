...
| **LLM09** | Misinformation | Модель генерирует убедительную, но фактически неверную информацию (перекрёстная ссылка: [Глава 3](03_галлюцинации.md)) |
...
Как мы видели в [Главе 13 (§13.5)](13_антигаллюцинационный_контур.md), защита строится в три уровня. Здесь разберём каждый подробнее.
...
Как мы видели в [Главе 10 (§10.3)](10_агент_не_чат.md), каждый tool call — потенциальный вектор атаки. Модель может быть уговорена (через injection или jailbreak) вызвать инструмент с вредоносными параметрами: удалить файлы, отправить данные на внешний сервер, выполнить произвольный код.
...
Для деструктивных и дорогостоящих операций — обязательное подтверждение человеком (как мы обсуждали в [§10.8](10_агент_не_чат.md)):
...
В [Главе 3](03_галлюцинации.md) мы разобрали механику галлюцинаций — модель генерирует правдоподобный, но несуществующий текст. В контексте безопасности это свойство становится attack vector.
...
Архитектурный паттерн guardrails (input guard → LLM → output guard) и общая логика фильтрации подробно описаны в [Главе 13 (§13.5)](13_антигаллюцинационный_контур.md). Здесь сфокусируемся на **security-специфичных** аспектах: какие фреймворки доступны и какие проверки критичны с точки зрения безопасности, а не качества.
...
Для quality-ориентированных guardrails (schema validation, hallucination markers, confidence check) — см. [Главу 13 (§13.5)](13_антигаллюцинационный_контур.md).
