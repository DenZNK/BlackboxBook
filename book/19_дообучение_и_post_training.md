...
Эта глава объясняет post-training pipeline — SFT, RLHF, DPO, LoRA — чтобы вы могли принимать обоснованные решения: когда дообучение оправдано, а когда проще переписать промпт. Цель — не превратить вас в ML-исследователя, а дать достаточно понимания, чтобы разговаривать с ML-командой на одном языке и принимать архитектурные решения.
...
Вместо трёхэтапного конвейера DPO использует одну целевую функцию. Для каждого промпта $x$ есть два ответа: предпочтённый $y_w$ и отвергнутый $y_l$:
...
|----------------|-----------|-----|
| Количество этапов | 3 (SFT → RM → PPO) | 1 (SFT → DPO) |
| Reward model | Нужна отдельная | Не нужна |
| Стабильность | Чувствителен к гиперпараметрам | Стабильнее |
| GPU-память | Хранит RM + Policy + Reference | Policy + Reference |
...
## 19.5. LoRA и QLoRA: parameter-efficient fine-tuning
...
```python
# Пример конфигурации LoRA (Hugging Face PEFT)
...
## 19.6. Синтетические данные для fine-tuning
...
## 19.7. Distillation: от дорогой модели к дешёвой
...
## 19.8. Дерево решений: промпт, RAG или fine-tuning?
...
| Нужен domain-specific reasoning | **Reinforcement fine-tuning** | Рассуждение через reward signal |
...
## 19.9. Failure modes: что может пойти не так
...
3. **Рассматривайте fine-tuning только когда промпт и RAG не справляются.** Типичные показания: стиль/тон на масштабе, preference alignment, distillation для снижения стоимости.
