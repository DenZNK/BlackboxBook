...
Anthropic к 2026 году сделала XML-теги **центральным элементом** своей системы prompt engineering для Claude. Это не просто рекомендация — это стандарт: системные промпты Claude, инструкции к инструментам, конституциональные правила — всё обёрнуто в XML. Внутренние бенчмарки показывают:
...
(По данным документации Anthropic; точные числа из внутренних бенчмарков публично не раскрыты.)

OpenAI пошла другим путём: их системные промпты GPT-4o и o3 используют **Markdown-секции** и внутренние конвенции (заголовки, нумерованные списки, блоки с тройным дефисом). JSON Schema применяется для structured output и описания инструментов. Оба подхода работают — но для разных моделей по-разному (см. 7.2).
...
| **GPT-4o / o3 (OpenAI)** | Markdown + JSON | Системные промпты OpenAI — Markdown-секции, инструменты — JSON Schema | `## Instructions\n...\n## Data\n...` |
...
Управлять этим можно так же, как зависимостями в коде:

```python
class PromptModule:
    """Модуль промпта с декларативными зависимостями."""
    def __init__(self, tag: str, content: str, depends_on: list[str] = None):
        self.tag = tag
...
class PromptAssembler:
    ...
    def register(self, module: PromptModule):
        self.modules[module.tag] = module

    def validate(self) -> list[str]:
        """Проверяет, что все зависимости удовлетворены."""
        errors = []
        for tag, mod in self.modules.items():
            for dep in mod.depends_on:
                if dep not in self.modules:
                    errors.append(f"Модуль <{tag}> зависит от <{dep}>, но тот не зарегистрирован")
        return errors

    def build(self) -> str:
        """Собирает промпт в топологическом порядке зависимостей."""
        # ... топологическая сортировка и сборка ...

# Использование:
assembler = PromptAssembler()
assembler.register(PromptModule("role", "SQL expert"))
assembler.register(PromptModule("output_schema", '{"query": "...", "tables": [...]}'))
assembler.register(PromptModule("constraints", "Только SELECT, PostgreSQL 16", 
                                depends_on=["output_schema"]))
assembler.register(PromptModule("examples", "...", 
                                depends_on=["output_schema", "constraints"]))

errors = assembler.validate()  # Пусто — все зависимости есть
