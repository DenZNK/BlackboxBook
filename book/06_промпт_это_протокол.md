...
OpenAI автоматически кэширует повторяющиеся префиксы промптов на поддерживаемых моделях. Для cached input tokens OpenAI отдельно указывает скидку до 90%.
