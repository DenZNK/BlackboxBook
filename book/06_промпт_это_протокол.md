...
Начиная с 2025 года OpenAI автоматически кэширует повторяющиеся префиксы промптов. Кэшированные токены стоят на 50% дешевле.
...
2. **Минимальный порог** — кэширование включается при ~1024+ токенов совпадающего префикса.
...
На типичном продакшн-нагрузке prompt caching снижает стоимость входных токенов на **50–90%**.
