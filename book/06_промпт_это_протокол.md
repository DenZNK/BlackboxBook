...
К 2026 году function calling (tool use) — **стандартная возможность** всех фронтир-моделей: GPT-5.4, Claude Opus 4.6 / Sonnet 4.6, Gemini 3.1 Pro, Mistral Large, Command R+.
...
OpenAI автоматически кэширует повторяющиеся префиксы промптов на поддерживаемых моделях. Для cached input tokens OpenAI предлагает скидку 50% (на октябрь 2024; проверяйте актуальные тарифы). Anthropic отдельно предлагает до 90% скидки на кэшированные токены.
